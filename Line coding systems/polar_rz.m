% polar_rz() generates a polar RZ (return to zero) pulse waveform from
% a sequence of binary values (0 or 1).
%
% Usage: signal = polar_rz(bits, high_voltage_level, low_voltage_level, samples_per_bit)
%
% Input arguments:
...
%            a polar RZ pulse waveform with the specified voltage levels and
...
%   low_voltage_level = 0;
%   samples_per_bit = 100;
%   signal = polar_rz(bits, high_voltage_level, low_voltage_level, samples_per_bit);
%
% The `polar_rz()` function generates a polar RZ pulse waveform by computing
...
% is also plotted using the `plot()` function within the `polar_rz()`
...
% using the `plot()` function within the `polar_rz()` function, and it is also returned as the output of the function.

function y = unipolar_rz(bits, high_voltage_level, samples_per_bit)
% polar RZ encoding of a binary sequence
